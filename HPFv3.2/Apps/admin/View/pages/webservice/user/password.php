...
if(true){
    $user = users::getBy(["user_id" => $_SESSION["user_id"], "user_password" => F::Encrypt(Input::post("old"))]);
    
	if(!empty(Input::post("old")) && !empty(Input::post("new1")) && !empty(Input::post("new2"))){
		
		if(count($user) > 0){
	        if(Input::post("new1") == Input::post("new2")){
	            users::updateBy(["user_id" => $_SESSION["user_id"]], ["user_password" => F::Encrypt(Input::post("new1"))]);
