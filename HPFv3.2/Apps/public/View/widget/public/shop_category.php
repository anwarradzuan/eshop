<div class="isotope-grid cols-3 mb-2" id="view_sorting">
	...
	<?php
		...
		$cat_key = url::get(1);
		$category = category::getBy(["c_name" => $cat_key]);
		...
		//$x = items::getBy(["i_category" => $cat_id]);
		$x = DB::conn()->q("SELECT * FROM items WHERE i_category = $cat_id AND i_status = 1 AND i_company IN (SELECT c_id FROM company WHERE c_status = 1)")->results();
		$total_item = count($x);
		if($total_item < 2){
			$page_total = 1;
		}else{
		...
		$item = DB::conn()->q("SELECT * FROM items WHERE i_category = $cat_id AND i_status = 1 AND i_company IN (SELECT c_id FROM company WHERE c_status = 1) LIMIT $position, $limit")->results();
		$no = $position + 1;
				...
				$t = $t[0];
					...
					<div class="grid-item">
						<div class="product-card">
							...
							<h3 class="product-title"><a href="<?= PORTAL ?>categories/view_item/<?= $y->i_key ?>"><?= F::trimWord($y->i_name, 24)?></a></h3>
						...
						<div class="product-card">
							...
							<div class="product-buttons">
								<?php
									...
									$w = wishlist::getBy(["w_item" => $ipid, "w_customer" => $cust]); 
									
									if(count($w) > 0){
										$w = $w[0];
										?>
											 <button class="btn btn-outline-secondary btn-sm whishlist-add" title="Whishlist" data-item="<?= $ikey ?>">
												<span id="view_wish"><i class="icon-heart" style="color:red;" ></i></span>
											 </button>
										<?php
									}else{
										?>
...
<?php
	...
	if($page > 1){
		...
		<div class="column text-left hidden-xs-down">
			<a class="btn btn-outline-secondary btn-sm" href="<?php echo PORTAL?>categories/<?= $cat_key ?>/page/<?= $prev ?>">Prev&nbsp;<i class="icon-arrow-left"></i></a>
		</div>
		<?php
	}else{
		?>
	...
	if($i != $page){
		?>
			<li class=""><a href="<?php echo PORTAL?>categories/<?= $cat_key ?>/page/<?= $i ?>"><?= $i ?></a></li>
		<?php
	}else{
		?>
			<li class="active"><a href="<?php echo PORTAL?>categories/<?= $cat_key ?>/page/<?= $i ?>"><?= $i ?></a></li>
		<?php
	}
	?>
		</ul>
	...
	if($page < $total){
		...
		<div class="column text-right hidden-xs-down">
			<a class="btn btn-outline-secondary btn-sm" href="<?php echo PORTAL?>categories/<?= $cat_key ?>/page/<?= $next ?>">Next&nbsp;<i class="icon-arrow-right"></i></a>
		</div>
		<?php
