...
<?php
	...
	// $item = items::list(["limit" = $limit]);
	$item = DB::conn()->q("SELECT * FROM items LIMIT $position, $limit")->results();
	$no = $position + 1;
			...
			if(count($t) > 0){
				...
				<div class="grid-item">
					<div class="product-card">
						...
						<a class="product-thumb" href="<?= PORTAL ?>shop/view_item/<?= $y->i_key ?>">
							<img src="<?= PORTAL_BUSINESS ?>assets/medias/iecommerce/img/shop/products/<?= $t->ip_file?>" alt="Product">
						</a>
						<h3 class="product-title"><a href="<?= PORTAL ?>shop/view_item/<?= $y->i_key ?>"><?= F::trimWord($y->i_name, 24)?></a></h3>
						...
						<div class="product-buttons" id="view_wish">
							...
							<a class="btn btn-outline-primary btn-sm add-cart" href="<?= PORTAL ?>shop/view_item/<?= $y->i_key ?>">View Product</a>
						</div>
					</div>
				</div>
				<?php	
...
<?php
	...
	if($page > 1){
		...
		<div class="column text-left hidden-xs-down">
			<a class="btn btn-outline-secondary btn-sm" href="<?php echo PORTAL?>shop?page=<?= $prev ?>">Prev&nbsp;<i class="icon-arrow-left"></i></a>
		</div>
		<?php
	}else{
		?>
	...
	if($i != $page){
		?>
			<li class=""><a href="<?php echo PORTAL?>shop?page=<?= $i ?>"><?= $i ?></a></li>
		<?php
	}else{
		?>
			<li class="active"><a href="<?php echo PORTAL?>shop?page=<?= $i ?>"><?= $i ?></a></li>
		<?php
	}
	?>
		</ul>
	...
	if($page < $total){
		...
		?>
			<div class="column text-right hidden-xs-down">
				<a class="btn btn-outline-secondary btn-sm" href="<?php echo PORTAL?>shop?page=<?= $next ?>">Next&nbsp;<i class="icon-arrow-right"></i></a>
			</div>
		<?php
