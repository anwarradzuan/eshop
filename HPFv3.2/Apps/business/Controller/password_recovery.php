...
if(!empty(Input::POST("email")) AND !empty(Input::POST("password")) AND !empty(Input::POST("key"))){
	...
	if(count($keys) > 0){
		...
		if($key->pr_expired > F::GetTime() AND $key->pr_status == 0){
			...
			new Alert("error", "Requested URL has expired. Please request new URL.", "border-radius: 10px; font-weight: bold; position:absolute; top: 10%; z-index: 5000; background-color: rgba(200, 95, 95, 0.9); left: 50%; transform: translate(-50%) ", "text-center col-md-6");
			echo "<a class='btn btn-danger btn-sm'	href='" . PORTAL . "'><span class='fa fa-arrow-left'></span> Got To Home</a>";
		}
	}else{
		new Alert("error", "Sorry, requested URL not found in our database. Please make sure you are using a correct URL to access this page.", "border-radius: 10px; font-weight: bold; position:absolute; top: 10%; z-index: 5000; background-color: rgba(200, 95, 95, 0.9); left: 50%; transform: translate(-50%) ", "text-center col-md-6");
	}
}else{
	new Alert("error", "Please fill all fields below.", "border-radius: 10px; font-weight: bold; position:absolute; top: 10%; z-index: 5000; background-color: rgba(200, 95, 95, 0.9); left: 50%; transform: translate(-50%) ", "text-center col-md-6");
}